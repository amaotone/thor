...
| レイヤー | ファイル | 役割 |
|----------|----------|------|
| Chat | `index.ts`, `discord.ts` | Discordクライアント、メッセージルーティング |
...
| Scheduler | `scheduler.ts`, `schedule-cli.ts` | cron/一回限りスケジュール、YAML永続化 |
| Skills | `skills.ts` | `workspace/skills/` からスキル読み込み |
| Config | `config.ts`, `settings.ts`, `sessions.ts` | 環境変数、ランタイム設定、セッション管理 |
