...
export interface DiscordToolsDeps {
  isSendableChannel?: (ch: unknown) => ch is SendableChannel;
  channelTypeGuildText?: number;
}

export function createDiscordTools(
  client: Client,
  runContext: RunContext,
  deps?: DiscordToolsDeps
): ToolDefinition[] {
  const isSendableChannel = deps?.isSendableChannel ?? defaultIsSendableChannel;
  const channelTypeGuildText = deps?.channelTypeGuildText ?? ChannelType.GuildText;
    ...
    handler: async () => {
      try {
        ...
        const channels = guild.channels.cache
          .filter((c) => c.type === channelTypeGuildText)
