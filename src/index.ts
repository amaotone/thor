...
function formatChannelStatus(
  ...
  const busy = processingChannels.has(channelId);
...
async function main() {
  ...
  if (discordAllowed.length > 1 || slackAllowed.length > 1) {
    console.error('[thor] Error: Only one user per platform is allowed');
  ...
  const commands: ReturnType<SlashCommandBuilder['toJSON']>[] = [
    ...
    new SlashCommandBuilder().setName('status').setDescription('現在の実行状態を表示する').toJSON(),
  ...
  client.once(Events.ClientReady, async (c) => {
    ...
    try {
      ...
      console.log(`[thor] Found ${guilds.size} guilds`);

      for (const [guildId, guild] of guilds) {
        await rest.put(Routes.applicationGuildCommands(c.user.id, guildId), {
          body: commands,
        });
        console.log(`[thor] ${commands.length} slash commands registered for: ${guild.name}`);
      }

      // グローバルコマンドをクリア（重複防止）
      await rest.put(Routes.applicationCommands(c.user.id), { body: [] });
      console.log('[thor] Cleared global commands');
    } catch (error) {
      console.error('[thor] Failed to register slash commands:', error);
    }
  });
  ...
  async function fetchReplyContent(message: Message): Promise<string | null> {
    ...
    try {
      ...
      console.log(`[thor] Fetched reply-to message from ${author}`);
      return `\n---\n💬 返信元 (${author}):\n${content}${attachmentInfo}\n---\n`;
    } catch (err) {
      console.error(`[thor] Failed to fetch reply-to message:`, err);
      return null;
    }
  }
  ...
  async function handleDiscordCommand(
    ...
    if (searchMatch) {
      ...
      try {
        ...
        console.error(`[thor] Failed to search messages`, err);
    ...
    if (editMatch) {
      ...
      try {
        ...
        if (channel && 'messages' in channel) {
          ...
          console.log(`[thor] Edited message ${messageId} in channel ${editedChannelId}`);
        ...
        console.error(`[thor] Failed to edit message:`, err);
    ...
    while (i < lines.length) {
      ...
      if (editMatch) {
        ...
        if (fullContent) {
          ...
          console.log(
            `[thor] Processing discord edit from response: ${commandText.slice(0, 50)}...`
          );
          const result = await handleDiscordCommand(commandText, sourceMessage, fallbackChannelId);
          if (result.handled && result.response) {
            if (result.feedback) {
  ...
  client.on(Events.MessageCreate, async (message) => {
    ...
    if (!isMentioned && processingChannels.has(message.channel.id) && !isControlCommand) {
      console.log(`[thor] Skipping message in busy channel: ${message.channel.id}`);
      return;
    }
  ...
  if (config.slack.enabled) {
    ...
    console.log('[thor] Slack bot started');
  }

  if (!config.discord.enabled && !config.slack.enabled) {
    console.error(
      '[thor] No chat platform enabled. Set DISCORD_TOKEN or SLACK_BOT_TOKEN/SLACK_APP_TOKEN'
    );
    process.exit(1);
  }
  ...
  // シャットダウン時にスケジューラを停止
  const shutdown = () => {
    console.log('[thor] Shutting down scheduler...');
    scheduler.stopAll();
    process.exit(0);
  };
  ...
  for (const cmd of commands) {
    ...
    if (setMatch) {
      ...
      if (key === 'autoRestart') {
        ...
        console.log(`[thor] autoRestart ${enabled ? 'enabled' : 'disabled'} by agent`);
      }
    }
  }
}
