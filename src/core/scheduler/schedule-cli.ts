 ...
 *
 * Usage:
 *   schedule-cli add --channel <id> --platform <discord> "30分後 メッセージ"
 *   schedule-cli add --channel <id> --platform <discord> --cron "0 9 * * *" --message "おはよう"
 *   schedule-cli list [--channel <id>] [--platform <discord>]
 *   schedule-cli remove <id>
 *   schedule-cli toggle <id>
 */

import { resolvePath } from '../shared/config.js';
...
Usage:
  schedule-cli add --channel <id> --platform <discord> "<入力>"
  schedule-cli add --channel <id> --platform <discord> --cron "<cron式>" --message "<メッセージ>"
  schedule-cli add --channel <id> --platform <discord> --at "<ISO日時>" --message "<メッセージ>"
  schedule-cli list [--channel <id>] [--platform <discord>]
  schedule-cli remove <id>
  schedule-cli toggle <id>
...
function main(): void {
  ...
  switch (command) {
    ...
    case 'toggle': {
      ...
      if (!Number.isNaN(indexNum) && indexNum > 0 && !targetId.startsWith('sch_')) {
        const channel = args.channel;
        const platform = args.platform as Platform | undefined;
        const schedules = scheduler.list(channel, platform);
